...
public class SelectElement extends Element {
     ...
     * @param selectorType io.github.sentinel.enums.SelectorType INDEX, VALUE, TEXT
    ...
    public boolean hasOption(String text) {
    	Select selectElement = new Select(this.element());
    	return new WebDriverWait(WebDriverFactory.getWebDriver(), Duration.ofSeconds(Time.out().toSeconds()), Duration.ofMillis(Time.interval().toMillis()))
