...
public class Element {
		...
		for (String file : fileLocators) {
			...
			try {
				...
				if (fileToProcess.exists() && !fileToProcess.isDirectory()) {
					...
					log.error(errorMessage);
					throw new io.github.sentinel.exceptions.IOException(errorMessage);
				}
			}
		}
	...
	public boolean isDisplayed() {
		try {
			return new WebDriverWait(driver(), Duration.ofSeconds(Time.out().toSeconds()), Duration.ofMillis(Time.interval().toMillis()))
				.ignoring(StaleElementReferenceException.class)
	...
	public Boolean waitForText(String text, boolean present) {
		...
		while ((System.currentTimeMillis() - startTime) < searchTime) {
			try {
				return new WebDriverWait(driver(), Duration.ofMillis(Time.interval().toMillis()), Duration.ofMillis(Time.loopInterval().toMillis()))
						.ignoring(StaleElementReferenceException.class)
